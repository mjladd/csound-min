...
Everything the compiler reads lives under `src/`. The top level holds only the
...
src/Engine/             the orchestra language and the scheduler
src/OOps/               base unit generators
src/Opcodes/            the opcode library
src/InOut/              audio, MIDI and file IO
src/Top/                the API surface and the performance loop
...
1. `src/frontend/csound_main.c` (239 lines). The whole command line
   program. It parses nothing and computes nothing. It calls the C API.
2. `src/Top/main.c`. `csoundCompile` at line 217 is the command line compile
   path. It calls `argdecode`, splits a `.csd` file, compiles the orchestra,
   and sorts the score.
3. `src/Top/csound_perf.c` (497 lines). The performance loop. `kperf` at line 241
   is the function that produces every sample this program ever writes.
4. `src/Engine/insert.c` (3,012 lines). Instrument instances. `insert_event` at
   line 895 starts a note. `init0` at line 765 sets up instrument 0.

`src/Top/csound.c` (2,437 lines) holds the rest of the public API, including
...
`src/include/csoundCore.h` line 537 defines `OPDS`, the header that sits at the
...
strings, and the three function pointers. `src/Engine/entry.c` line 78 opens
`opcodlst_1[]`, the table of 1,543 built-in entries. Each opcode file under
`src/Opcodes/` declares its own `localops[]` table in the same shape.

## The directories

| Directory | Lines | What it holds |
| --- | --- | --- |
| `src/Engine/` | 38,664 | The orchestra language: preprocessor, lexer, parser, type system, and the code that turns a parse tree into opcode chains. Also the score reader and the scheduler. |
| `src/OOps/` | 52,556 | The base unit generators, the ones that predate the plugin system. `ugens1.c` through `ugens6.c`, `aops.c` for arithmetic, `str_ops.c` for strings. |
| `src/Opcodes/` | 125,179 | The opcode library, 249 files. Each file is close to self-contained. This is the largest directory and the easiest one to ignore until you need one specific opcode. |
| `src/Top/` | 16,847 | The library boundary. The public API, argument parsing, message routing, the performance loop. |
| `src/InOut/` | 18,401 | Real-time audio and MIDI back ends, sound file reading and writing, and the built-in graph display. |
| `src/include/` | 10,965 | Public headers. `csound.h` is the API, 127 functions. `csoundCore.h` is the internal one, and it holds the `CSOUND` struct. |
| `src/H/` | 10,717 | Internal headers, mostly per-opcode state structure definitions. |
| `src/util/` | 15,681 | Standalone analysis programs: `pvanal`, `hetro`, `lpanal`, `atsa`, `dnoise`, `srconv`. Also built as the `stdutil` plugin, which is what `csound -U <name>` loads. |
...
2. `src/Top/main.c` `csoundCompile` at line 217 reads the flags through
   `argdecode`, then calls `read_unified_file4` at line 296 to split a `.csd`
   file into its orchestra and score parts.
3. `src/Engine/csound_pre.lex` runs the preprocessor, which handles `#include`
   and macros. It is a separate Flex lexer of 1,566 lines.
4. `src/Engine/csound_orc.lex` and `src/Engine/csound_orc.y` produce a parse tree.
   `src/Engine/new_orc_parser.c` `csoundParseOrc` at line 103 drives them.
5. `src/Engine/csound_orc_semantics.c` resolves types and picks, for each opcode
   call, which `OENTRY` matches the argument types.
6. `src/Engine/csound_orc_compile.c` turns the tree into `OPTXT` templates, one
   per opcode call, grouped per instrument.
7. `src/Engine/sread.c` reads the score. `scsortstr` sorts the events by time.
8. `src/Engine/musmon.c` runs the event loop. `sense_events` at line 1268 finds
   the events that are due.
9. `src/Engine/insert.c` `insert_event` at line 895 allocates an instance from
   the templates and links its opcodes into the active chain.
10. `src/Top/csound_perf.c` `kperf` at line 241 walks the active chain and calls
    `perf` on every opcode, ksmps samples at a time.
11. `csound->spout` holds the result. `src/InOut/` writes it to a file or a
    device.

## What the engine keeps its state in

`src/include/csoundCore.h` line 1091 opens `struct CSOUND_`. It has 529 fields,
